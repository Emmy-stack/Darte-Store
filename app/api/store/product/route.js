...
export async function POST (request){
    try {
        ...
        const price = Number.isFinite(priceInput) && priceInput > 0 ? priceInput : mrp

        if (!name || !description || !mrp || !category || images.length < 1) {
            return NextResponse.json({ error: "missing product details" }, { status: 400 })
        }

        if (mrp <= 0) {
            return NextResponse.json({ error: "invalid actual price" }, { status: 400 })
        }
