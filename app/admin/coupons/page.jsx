...
export default function AdminCoupons() {
    ...
    const handleAddCoupon = async (e) => {
        ...
        try {
            const token = await getToken()
            const { data } = await axios.post('/api/admin/coupons', newCoupon, {
                headers: { Authorization: `Bearer ${token}` }
            })
    ...
    const deleteCoupon = async (code) => {
        const token = await getToken()
        const { data } = await axios.delete(`/api/admin/coupons/${code}`, {
            headers: { Authorization: `Bearer ${token}` }
        })
        fetchCoupons() // Refresh the list
        return data;
    ...
    const confirmAndDeleteCoupon = (code) => {
        if (window.confirm(`Are you sure you want to delete the coupon "${code}"?`)) {
            toast.promise(deleteCoupon(code), {
                loading: "Deleting coupon...",
                success: (data) => data.message || "Coupon deleted successfully",
                error: (err) => err?.response?.data?.error || "Failed to delete coupon"
            })
        }
    }
                            ...
                            {coupons.map((coupon) => (
                                <tr key={coupon.code} className="hover:bg-slate-50">
                                    ...
                                    <td className="py-3 px-4 text-slate-800">
                                        <DeleteIcon onClick={() => confirmAndDeleteCoupon(coupon.code)} className="w-5 h-5 text-red-500 hover:text-red-800 cursor-pointer" />
                                    </td>
                                </tr>
                            ))}
                        </tbody>
