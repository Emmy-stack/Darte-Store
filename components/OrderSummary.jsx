...
const OrderSummary = ({ totalPrice, items }) => {
    ...
    const handleCouponCode = async (event) => {
        ...
        try {
            ...
            setCouponCodeInput('');
            toast.success(`Coupon ${data.coupon.code.toUpperCase()} applied!`);
        } catch (error) {
            ...
            <div className='pb-4 border-b border-slate-200'>
                <div className='flex justify-between'>
                    ...
                    <div className='flex flex-col gap-1 font-medium text-right'>
                        <p>{currency}{formatPrice(totalPrice)}</p>
                        <p>Free</p>
                        {coupon && <p>{`-${currency}${formatPrice((coupon.discount / 100) * totalPrice)}`}</p>}
                    </div>
                </div>
                {
                    !coupon ? (
