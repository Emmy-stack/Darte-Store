...
async function run() {
    console.log("Starting subaccount listing & self-healing split updates (99% / NGN)...");
    ...
    for (const pa of payoutAccounts) {
        ...
        const targetSplitValue = 99.0;
        const targetSplitType = "percentage";
        const targetFlwSplitValue = 0.99; // 99%
        ...
        if (flwMatched) {
            ...
            if (!updatedSubaccountsOnFlw.has(numericId)) {
                console.log(`Updating subaccount (numeric_id=${numericId}) on Flutterwave to 99% split and NGN currency...`);
                try {
